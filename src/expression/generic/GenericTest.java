...
public class GenericTest extends BaseTest {
    ...
    final List<Op<F<?>>> tests = new ArrayList<>();
    ...
    BigInteger bi(final int x) {
        return BigInteger.valueOf(x);
    }

    Integer i(final long x) {
        return Integer.MIN_VALUE <= x && x <= Integer.MAX_VALUE ? (int) x : null;
    }

    void all(final String expression, final F<Integer> fi, final F<Double> fd, final F<BigInteger> fb) {
